...
| `ID/EX`      | `后三个阶段的控制信号(IR),(NPC),由指令译码/扩展、访问寄存器的数据(A,B,Imm)` |
| `EX/MEM`     | `MEM和WB的控制信号(IR),ALU计算结果(ALUOut)和Zero结果(cond),PC+4+Offset(NPC')` |
| `MEM/WB`     | `WB的控制信号(IR),EX阶段的ALUOut(ALUOut),访存结果(MDR)`      |
